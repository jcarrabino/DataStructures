...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//stack of reachable vertices
	Deque* vStack = dequeNew();
	//used to compare values stored in deque
	Vertex* currentV;
	dequePushBack(vStack, source);
	int i;
	
	while(!dequeIsEmpty(vStack) && !found){
		//type casting due to deque's use of TYPE void*
	    currentV = (Vertex*)dequeBack(vStack);
	    dequePopBack(vStack);
   
		currentV->isVisited = 1;
		if(destination == currentV){
			found = 1; 
			break;
		}
	   
	    for(i = 0; i < currentV->numNeighbors; i++){
			if(currentV->neighbors[i]->isVisited == 0)
			   dequePushBack(vStack, currentV->neighbors[i]);
		}
    }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	Deque* vQueue = dequeNew();
	Vertex* currentV;
	dequePushFront(vQueue, source);
	int i;
	
	while(!dequeIsEmpty(vQueue) && !found){
	    currentV = (Vertex*)dequeFront(vQueue);
	    dequePopFront(vQueue);
	   ...
	   //push neighbors of current vertex to back of queue
	    for(i = 0; i < currentV->numNeighbors; i++){
			if(currentV->neighbors[i]->isVisited == 0)
			   dequePushBack(vQueue, currentV->neighbors[i]);
		}
    }
